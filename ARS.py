...
#   
#   Purpose: 
#           Class for Adaptive Rejection Sampling (ARS), follwoing from 
#           Gilks, Walter R., and Pascal Wild. "Adaptive rejection sampling for Gibbs sampling." 
#           with code adapted from https://github.com/junlulocky/ARS-MCMC
#
##############################################################################################
...
class ARS():
    ...
    def __init__(self, f, fprima = None, xi=[-4,1,4], lb=-np.inf, ub=np.inf, use_lower=False, ns=50, **fargs):
        '''
        ...
        '''
        def num_deriv(x, delta = 1e-8, **fargs): 
                return (f(x + delta, **fargs) - f(x, **fargs))/(delta)
        ...
        if not(self.hprime[0] > 0): 
            warnings.warn("First value of h prime must be postive")
        if not(self.hprime[-1] < 0):
            warnings.warn("Last value of h prime must be negative")
        self.insert() 
